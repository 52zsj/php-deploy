...
从 Git 仓库拉取代码并同步到多台服务器（支持 Gitee / GitHub / GitLab 等）。

原名「Gitee 代码同步工具 / php-deploy」，产品标识已统一为 **GitShip**。

## ⭐ v2.0 核心特性

### 🎯 以 Git 为真相源
- **自动保护插件文件**：只操作 Git 管理的文件，插件生成的文件自动被保护
- **智能删除**：Git 删除的文件，服务器也删除；不在 Git 中的文件，完全不受影响
- **零配置保护**：不需要手动配置 `protect_dirs`

### 🚀 智能比对
- **rsync --checksum**：比对文件内容，只上传有变化的文件
- **性能提升 85%+**：小改动 2 秒完成（之前 30 秒）
- **首次部署优化**：智能跳过已存在且相同的文件
...
就这么简单！不需要 `enable_incremental`、`sync_mode`、`protect_dirs` 等复杂配置。
